// $Id: ODLCompiler.java,v 1.9 1999/02/11 18:11:22 matt Exp $
	...
	implements Runnable
{
	...
	/**
	 *  A list of the script files generated.
	 */
	protected Vector outFiles = new Vector ();
	
	/**
	 *  An object that receives any error messages. Messages
	 *  are also always sent to <tt>System.err</tt>.
	 */
	protected Eavesdropper listener = null;
...
//		}
	...
	/**
	 *  Compile a single named ODL source file and return
	 *  a list of the script files generated.
	 */
	public Vector compile ( String fileName )
	{
		outFiles = new Vector ();
		reset();
		
		readODL ( fileName );
		writeScripts ( fileName );
		
		Vector result = outFiles;
		outFiles = new Vector ();
		reset();
	...
	/**
	 *  Compile a list of named ODL source files and return
	 *  a list of the script files generated.
	 */
	public Vector compile ( Vector fileNames )
	{
		outFiles = new Vector ();
		
		Enumeration fileList = fileNames.elements();
	...
	public void readODL ( String fileName )
	{
		...
		{
			if ( in == null )
				printErr ( "Exception processing file "
						   + fileName + ": " + x );
			else
				printErr ( "Exception processing file "
						   + fileName
	...
	public void writeScripts ( String fileName )
	{
		...
		// get the short name (ie, without any path info) of the
		// file to use in the #source directive
		...
		if ( atoms.size() > 0 )
		{
			...
			try
			{
				...
				// write file header
				atomFile.println ( "#source " + srcName );
				...
				outFiles.addElement ( baseName + ".atom" );
			}
			catch ( IOException e )
			{
				printErr ( "Exception writing atom file "
						   + baseName + ".atom :" + e );
			}
		}
		...
		if ( things.size() > 0 )
		{
			...
			try
			{
				...
				// write file header
				thingFile.println ( "#source " + srcName );
				...
				outFiles.addElement ( baseName + ".thing" );
			}
			catch ( IOException e )
			{
				printErr ( "Exception writing thing file "
						   + baseName + ".thing :" + e );
			}
		}
		...
		if ( exits.size() > 0 )
		{
			...
			try
			{
				...
				// write file header
				exitFile.println ( "#source " + srcName );
				...
				outFiles.addElement ( baseName + ".exit" );
			}
			catch ( IOException e )
			{
				printErr ( "Exception writing exit file "
						   + baseName + ".exit :" + e );
			}
		}
	}
	...
	{
		System.err.println("ODLCompiler [" + VERSION + "] usage:");
	...
	/**
	 *  Print an error message. This always goes to <tt>System.err</tt>,
	 *  but a client class can also receive the messages if it calls
	 *  <tt>listen()</tt> appropriately.
	 */
	...
	/**
	 *  Set the listener that will receive any error messages.
	 *  The ODLCompiler only supports a single listener in
	 *  addition to <tt>System.err</tt>, so a call to this
	 *  function will always detach the previous listener,
	 *  and a call with <tt>null</tt> will stop listening
	 *  altogether.
	 */
	public void listen ( Eavesdropper listener )
	{
